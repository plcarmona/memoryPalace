...
import { act, extend, createRoot, _roots, reconciler } from '@react-three/fiber';
export { act } from '@react-three/fiber';
...
const toTreeBranch = children => children.map(child => {
  return treeObjectFactory(lowerCaseFirstLetter(child.object.type || child.object.constructor.name), child.props, toTreeBranch(child.children));
});
const toTree = root => toTreeBranch(root.children);
...
function processThreeChildren(children) {
  return children.map(object => graphObjectFactory(object.type, object.name || '', object.children && object.children.length > 0 ? processThreeChildren(object.children) : []));
}
const toGraph = object => {
  return object.children.map(child => {
    var _child$object$childre, _child$object$name;
    // Process standard R3F children
    const children = toGraph(child);

    // For primitives, also include THREE.js object children
    if (child.type === 'primitive' && (_child$object$childre = child.object.children) != null && _child$object$childre.length) {
      const threeChildren = processThreeChildren(child.object.children);
      children.push(...threeChildren);
    }
    return graphObjectFactory(child.object.type, (_child$object$name = child.object.name) != null ? _child$object$name : '', children);
  });
};
...
const findAll = (root, decider, options = {
  includeRoot: true
}) => {
  const results = [];
...
const createVirtualInstance = (object, parent) => {
  ...
  const instance = {
    ...
    // Convert to lowercase to match R3F convention
    parent,
    children: [],
    props: {
      object
...
class ReactThreeTestInstance {
  constructor(_fiber) {
    this.getChildren = (fiber, opts = {
      ...
      var _fiber$object$childre;
      // Get standard R3F children
      const r3fChildren = fiber.children.filter(child => !child.props.attach || opts.exhaustive).map(fib => wrapFiber(fib));

      // For primitives, also add THREE.js object children
      if (fiber.type === 'primitive' && (_fiber$object$childre = fiber.object.children) != null && _fiber$object$childre.length) {
        const threeChildren = fiber.object.children.map(child => {
    ...
    this.find = decider => expectOne(this.findAll(decider), `matching custom checker: ${decider.toString()}`);
    this.findByType = type => expectOne(this.findAll(node => Boolean(node.type && node.type === type)), `with node type: "${type || 'Unknown'}"`);
    this.findAllByType = type => this.findAll(node => Boolean(node.type && node.type === type));
    this.findByProps = props => expectOne(this.findAllByProps(props), `with props: ${JSON.stringify(props)}`);
    this.findAllByProps = props => this.findAll(node => Boolean(node.props && matchProps(node.props, props)));
    this._fiber = _fiber;
  }
  get fiber() {
    return this._fiber;
...
const create = async (element, options) => {
  ...
  const _scene = _store.getState().scene.__r3f;
  return {
...
export { index$1 as ReactThreeTest, create, index as default, waitFor };
